...
def list_pods_by_component_prefix(namespace: str, component_prefix: str) -> list[str]:
    """List Running pod names whose app.kubernetes.io/component starts with prefix."""
    try:
        result = run_oc_command([
            "get", "pods", "-n", namespace,
        ...
        for line in result.stdout.strip().splitlines():
            ...
            if len(parts) < 3:
                continue
            name, component, phase = parts[0], parts[1], parts[2]
            if component.startswith(component_prefix) and phase == "Running":
                names.append(name)
        return names
    except subprocess.CalledProcessError:
        return []
...
def cost_worker_pods_ready(namespace: str) -> bool:
    """Return True if at least one cost-worker-* pod is Running."""
    return len(list_pods_by_component_prefix(namespace, "cost-worker")) > 0

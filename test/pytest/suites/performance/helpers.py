...
def get_chart_version(namespace: str, release_name: str) -> str:
    """Get deployed stack version (CMSC annotation or resourceVersion)."""
    result = run_oc_command([
        "get", "cmsc", release_name,
        "-n", namespace,
        "-o", "jsonpath={.metadata.annotations.costmanagement\\.openshift\\.io/version}",
    ], check=False)
    ...
    if result.returncode == 0 and result.stdout.strip():
        return f"cmsc-rv-{result.stdout.strip()}"
...
def build_koku_api_url(cr_name: str, namespace: str) -> str:
    """Build the internal Koku API URL for in-cluster requests."""
    return (
        f"http://{cr_name}-koku-api"

...
def _collect_restart_baseline(namespace: str) -> Dict[str, int]:
    ...
    from utils import list_pods_by_component_prefix

    baseline: Dict[str, int] = {}
    for label in COMPONENT_LABELS:
        if label == "app.kubernetes.io/component=cost-worker":
            continue
        baseline.update(get_pod_restart_counts(namespace, label))
    for pod in list_pods_by_component_prefix(namespace, "cost-worker"):
        result = run_oc_command([
            "get", "pod", pod, "-n", namespace,
            "-o", "jsonpath={.status.containerStatuses[0].restartCount}",
        ], check=False)
        if result.returncode == 0 and result.stdout.strip().isdigit():
            baseline[pod] = int(result.stdout.strip())
    return baseline
...
def _collect_oomkill_events(namespace: str) -> List[Dict[str, str]]:
    ...
    for pod in list_pods_by_component_prefix(namespace, "cost-worker"):
        result = run_oc_command([
            "get", "pod", pod, "-n", namespace,
            "-o", "jsonpath={range .status.containerStatuses[*]}{.lastState.terminated.reason}{end}",
        ], check=False)
        if result.returncode == 0 and "OOMKilled" in result.stdout:
            events.append({"pod": pod, "reason": "OOMKilled"})
    return events
...
class TestStress:
    ...
    def setup(self, cluster_config: ClusterConfig, keycloak_config):
        ...
        self.cr_name = cluster_config.cr_name
        ...
        try:
            ...
            clb_result = run_oc_command(
                ["get", "pods", "-n", self.namespace,
                 "-l", f"app.kubernetes.io/instance={self.cr_name}",

...
class TestCeleryReplicaSweep:
    ...
    def test_perf_cel_001_replica_sweep(
        ...
        deploy_name = f"{self.cr_name}-{info['deployment_suffix']}"
...
class TestWorkerOOMThreshold:
    ...
    def test_perf_cel_002_worker_oom_threshold(
        ...
        deploy_name = f"{self.cr_name}-celery-worker-ocp"
...
class TestKruizeMultiReplica:
    ...
    def test_perf_ros_001_kruize_multi_replica(
        ...
        deploy_name = f"{self.cr_name}-kruize"
        original_replicas = get_deployment_replicas(self.namespace, deploy_name)
...
def capture_warm_state_indicators(
    ...
    for pod_name in list_pods_by_component_prefix(namespace, "cost-worker"):
        result = run_oc_command(
        ...
        if result.returncode == 0 and result.stdout.strip():
            try:
                ...
                worker_ages[pod_name] = round(age_s, 0)
            except (ValueError, TypeError):
                pass

    result = run_oc_command(
        ["get", "pods", "-n", namespace,
         "-l", f"app.kubernetes.io/instance={cr_name},app.kubernetes.io/component=listener",

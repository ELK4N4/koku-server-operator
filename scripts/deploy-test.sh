...
################################################################################
# Cost Management Operator deploy-test (adapted from cost-helm-chart deploy-test-cost-onprem.sh).
# Replaces Helm install with operator deploy + CostManagementServiceConfig (install-cmsc.sh).
#
...
SKIP_OPERATOR_DEPLOY="${SKIP_OPERATOR_DEPLOY:-false}"
SKIP_PUSH="${SKIP_PUSH:-false}"
UNDEPLOY_OPERATOR="${UNDEPLOY_OPERATOR:-false}"
DELETE_CMSC_AFTER_TESTS="${DELETE_CMSC_AFTER_TESTS:-true}"
...
# Default: test/pytest/perf-runs (absolute path to avoid issues when pytest runs from test/pytest/)
PERF_OUTPUT_DIR="${PERF_OUTPUT_DIR:-${PROJECT_ROOT}/test/pytest/perf-runs}"
...
SCRIPT_INSTALL_CMSC="install-cmsc.sh"
...
create_namespace() {
    ...
    if [[ "${DRY_RUN}" == "true" ]]; then
        log_info "DRY RUN: Would ensure operator namespace ${OPERATOR_NS}"
        return 0
    fi

    if ! oc get namespace "${OPERATOR_NS}" &> /dev/null; then
        oc create namespace "${OPERATOR_NS}"
        log_success "Created operator namespace: ${OPERATOR_NS}"
    fi
...
default_operator_img() {
    local tag="${IMG_TAG:-latest}"
    echo "image-registry.openshift-image-registry.svc:5000/${OPERATOR_NS}/koku-service-operator:${tag}"
}

registry_reachable_from_laptop() {
    local host
...
ensure_binary_buildconfig() {
    if oc get buildconfig/koku-service-operator -n "${OPERATOR_NS}" >/dev/null 2>&1; then
        return 0
    fi
    log_info "Creating Binary Docker BuildConfig/ImageStream in ${OPERATOR_NS}"
    oc new-build --binary --strategy=docker \
        --name=koku-service-operator \
        -n "${OPERATOR_NS}" \
        --to='koku-service-operator:latest'
}

build_and_push_operator_image() {
    local img="$1"
    if [[ "${SKIP_PUSH}" == "true" ]]; then
        log_warning "Skipping image build/push (--skip-push); using existing ${img}"
        return 0
    fi

    local external_registry=""
    if external_registry="$(registry_reachable_from_laptop)"; then
        local push_img="${external_registry}/${OPERATOR_NS}/koku-service-operator:${IMG_TAG:-latest}"
        log_info "Building with docker and pushing to exposed registry: ${push_img}"
        (
            cd "${PROJECT_ROOT}"
            make docker-build IMG="${push_img}"
            oc registry login --registry="${external_registry}" 2>/dev/null || true
    ...
    log_info "No exposed registry route; using oc start-build (Binary) into ImageStream"
    ensure_binary_buildconfig
    (
        cd "${PROJECT_ROOT}"
        oc start-build koku-service-operator \
            -n "${OPERATOR_NS}" \
            --from-dir=. \
            --follow \
            --wait
    )
    oc get istag "koku-service-operator:${IMG_TAG:-latest}" -n "${OPERATOR_NS}" >/dev/null 2>&1 \
        || oc tag "${OPERATOR_NS}/koku-service-operator:latest" \
            "${OPERATOR_NS}/koku-service-operator:${IMG_TAG:-latest}" 2>/dev/null \
        || true
    log_info "Image available as ${img}"
}

deploy_operator() {
    if [[ "${SKIP_OPERATOR_DEPLOY}" == "true" ]]; then
        log_warning "Skipping operator controller deploy (--skip-operator-deploy)"
        return 0
    fi

    log_step "Deploying koku-service-operator controller"

    if [[ "${DRY_RUN}" == "true" ]]; then
        log_info "DRY RUN: Would build/push operator image and run make deploy"
        return 0
    fi

    local img="${IMG:-}"
    if [[ -z "${img}" ]]; then
        img="$(default_operator_img)"
        log_info "IMG defaulted to cluster ImageStream pull URL: ${img}"
    fi
    export IMG="${img}"

    oc policy add-role-to-user system:image-puller \
        "system:serviceaccount:${OPERATOR_NS}:koku-service-operator-controller-manager" \
        -n "${OPERATOR_NS}" 2>/dev/null || true

    build_and_push_operator_image "${IMG}"
    # build_and_push may override IMG (e.g. external registry push URL)

    (
        cd "${PROJECT_ROOT}"
        log_info "Applying CRDs + controller manifests (make deploy IMG=${IMG})..."
        make deploy IMG="${IMG}"
    )
    # make deploy runs kustomize edit set image; restore committed kustomization.yaml
    if git -C "${PROJECT_ROOT}" rev-parse --is-inside-work-tree &>/dev/null; then
        git -C "${PROJECT_ROOT}" checkout -- config/manager/kustomization.yaml 2>/dev/null || true
    fi

    local deploy_name="koku-service-operator-controller-manager"
    if oc get deploy "${deploy_name}" -n "${OPERATOR_NS}" >/dev/null 2>&1; then
        log_info "Restarting ${deploy_name} to pick up ${IMG}..."
        oc rollout restart "deploy/${deploy_name}" -n "${OPERATOR_NS}"
        oc rollout status "deploy/${deploy_name}" -n "${OPERATOR_NS}" --timeout=600s
    fi

    log_success "Operator controller deployed"
}

resolve_rhbk_values_file() {
    if [[ -n "${RHBK_VALUES_FILE:-}" && -f "${RHBK_VALUES_FILE}" ]]; then
        echo "${RHBK_VALUES_FILE}"
        return 0
    fi
    local candidate
    for candidate in \
        "${PROJECT_ROOT}/config/samples/pytest/jwt-auth-values.yaml" \
        "${PROJECT_ROOT}/cost-onprem/values.yaml" \
        "${PROJECT_ROOT}/../insights-onprem/cost-helm-chart/cost-onprem/values.yaml" \
        "${PROJECT_ROOT}/../cost-helm-chart/cost-onprem/values.yaml"; do
        if [[ -f "${candidate}" ]]; then
            echo "${candidate}"
            return 0
        fi
...
deploy_rhbk() {
    ...
    local rhbk_args=()
    local chart_values=""
    if chart_values="$(resolve_rhbk_values_file)"; then
...
apply_sizing_profile() {
    log_warning "Sizing profiles (--sizing-profile) are Helm-only; ignored for operator CMSC deploy"
    return 0
}

deploy_helm_chart() {
    if [[ "${SKIP_HELM}" == "true" ]]; then
        log_warning "Skipping CostManagementServiceConfig install (--skip-helm)"
        return 0
    fi

    log_step "Installing CostManagementServiceConfig (operator workload)"

    export NAMESPACE="${NAMESPACE}"
    export CR_NAME="${CR_NAME}"
    export KEYCLOAK_NAMESPACE="${KEYCLOAK_NAMESPACE}"
    export KAFKA_NAMESPACE="${KAFKA_NAMESPACE}"
    export OPERATOR_NS="${OPERATOR_NS}"
    ...
    if ! execute_script "${SCRIPT_INSTALL_CMSC}"; then
        log_error "CMSC installation failed"
        log_info ""
        log_info "To troubleshoot:"
        log_info "  1. Check CMSC status: oc get cmsc ${CR_NAME} -n ${NAMESPACE}"
        log_info "  2. Check pod status: oc get pods -n ${NAMESPACE}"
        log_info "  3. Check operator logs: oc logs -n ${OPERATOR_NS} deploy/koku-service-operator-controller-manager --tail=100"
        exit 1
    fi

    log_success "CostManagementServiceConfig installation completed"
}
...
setup_tls() {
    ...
    if [[ ! -f "${LOCAL_SCRIPTS_DIR}/${SCRIPT_SETUP_TLS}" ]]; then
        log_warning "TLS setup script not found (${SCRIPT_SETUP_TLS}); skipping"
        return 0
    fi
...
run_tests() {
    ...
    log_step "Running operator pytest suite (6/6)"
    ...
    export NAMESPACE="${NAMESPACE}"
    export CR_NAME="${CR_NAME}"
    export HELM_RELEASE_NAME="${CR_NAME}"
    ...
    if ! "${pytest_script}" ${pytest_args[@]+"${pytest_args[@]}"}; then
        ...
        log_info "JUnit report available at: test/pytest/reports/junit.xml"
...
run_iqe_tests() {
    ...
    if ! "${iqe_script}" --namespace "${NAMESPACE}" --profile "${IQE_PROFILE}" --marker "${IQE_MARKER}"; then
        ...
        log_info "IQE JUnit report available at: test/pytest/reports/iqe_junit.xml"
        log_info "IQE output log available at: test/pytest/reports/iqe_output.log"
        test_result=1
    else
...
delete_cmsc() {
    if [[ "${DELETE_CMSC_AFTER_TESTS}" != "true" ]]; then
        return 0
    fi
    if [[ "${DRY_RUN}" == "true" ]]; then
        log_info "DRY RUN: Would delete CMSC ${CR_NAME} in ${NAMESPACE}"
        return 0
    fi
    log_step "Deleting CMSC ${CR_NAME}"
    oc delete cmsc "${CR_NAME}" -n "${NAMESPACE}" --wait=true --timeout=300s --ignore-not-found
    log_success "CMSC deleted"
}

undeploy_operator_if_requested() {
    if [[ "${UNDEPLOY_OPERATOR}" != "true" ]]; then
        return 0
    fi
    if [[ "${DRY_RUN}" == "true" ]]; then
        log_info "DRY RUN: Would run make undeploy"
        return 0
    fi
    log_step "Undeploying operator (make undeploy)"
    (cd "${PROJECT_ROOT}" && make undeploy ignore-not-found=true) || true
}
...
print_summary() {
    ...
    echo "  CMSC Name:           ${CR_NAME}"
    echo "  Operator Namespace:  ${OPERATOR_NS}"
    [[ "${DEPLOY_S4}" == "true" ]] && echo "  S4 Namespace:        ${S4_NAMESPACE}"
    [[ -n "${SIZING_PROFILE}" ]] && echo "  Sizing Profile:      ${SIZING_PROFILE} (Helm-only; ignored)"
    echo ""
    log_info "Steps to execute:"
    [[ "${SKIP_OPERATOR_DEPLOY}" == "false" ]] && echo "  ✓ Deploy koku-service-operator controller" || echo "  ✗ Deploy operator controller (SKIPPED)"
    ...
    [[ "${SKIP_HELM}" == "false" ]] && echo "  ✓ Install CostManagementServiceConfig" || echo "  ✗ Install CMSC (SKIPPED)"
    [[ "${SKIP_TLS}" == "false" ]] && echo "  ✓ Setup TLS Certificates" || echo "  ✗ Setup TLS Certificates (SKIPPED)"
    [[ "${SKIP_TEST}" == "false" ]] && echo "  ✓ Run Pytest Suite" || echo "  ✗ Run Pytest Suite (SKIPPED)"
    [[ "${DELETE_CMSC_AFTER_TESTS}" == "true" ]] && echo "  ✓ Delete CMSC after tests" || echo "  ✗ Leave CMSC installed after tests"
...
print_completion() {
    ...
    echo ""
    log_info "Cost Management operator stack deployed to namespace: ${NAMESPACE} (CMSC: ${CR_NAME})"
    echo ""
    log_info "Next steps:"
...
main() {
    echo ""
    echo -e "${CYAN}Cost Management Operator deploy-test${NC}"
    echo ""
    ...
    while [[ $# -gt 0 ]]; do
        case $1 in
            ...
            --skip-push)
                ...
                ;;
            --undeploy-operator)
                UNDEPLOY_OPERATOR=true
                shift
                ;;
            --keep-cmsc)
                DELETE_CMSC_AFTER_TESTS=false
                shift
                ;;
            --skip-tls)
                SKIP_TLS=true
    ...
    # Deploy observability after workload is installed so services exist for auto-detection
    if [[ "${DEPLOY_OBSERVABILITY}" == "true" ]]; then
